...
class ESP8266Command:
    """Класс для команд ESP8266"""
    def __init__(self, command: str, params: Dict[str, Any] = None, device_id: str = None, plant_id: str = None):
    ...
    def update_esp_device_status(self, device_id: str, status: str = 'online', 
        ...
        with self.lock:
            with sqlite3.connect(self.db_path) as conn:
                ...
                query = f"UPDATE esp_devices SET {', '.join(update_fields)} WHERE device_id = ?"
...
class MQTTManager:
    ...
    def handle_esp_data(self, device_id: str, data: Dict):
    ...
    def handle_esp_status(self, device_id: str, status_data: Dict):
...
def determine_plant_health_status(sensor_data: List[Dict], ml_result: Dict = None) -> tuple[str, str]:
    """
    ...
    if ml_result and ml_result.get('health_status') != PlantHealthStatus.HEALTHY.value:
...
def send_command_to_esp():
    ...
    try:
        ...
        esp_command = ESP8266Command(command, params, device_id, plant_id)
